...
contract mlp_cancer_classifier_2025 {
    ...
    constructor() {{
        ...
        datasetPolicies["42bbbcb3a2a184689eedd3f6cf251bb4d6f586e04b9c657d9d0b6fac1d52289f"] = "yes";
        datasetPolicies["5aecf378b9e6a1a284d0c29670759f21fbef49ec790ce854f49ab9b3f57168cb"] = "yes";
        datasetPolicies["2ef58caa109295dfeeaa5476497104ddd2b89994cdca7d5acf47f57363986c34"] = "yes";
        datasetPolicies["9adc2c43bddf7fcce92358d2208e45f9055d14c822f590a369332f60d8a89a55"] = "yes";
        datasetPolicies["fcd8b61a7b8aa07ce43188421998887decc9c67f8101f3c4d903e2f441c1e57e"] = "yes";
        datasetPolicies["5077d58a091dde671cf2029631d6e063c9aba52b2d3f438c518c1c6eb3a95939"] = "yes";
        datasetPolicies["89697f99c145037c70d68c7a94597c86978f92b9e033a60dd91621cffbb5c9c4"] = "yes";
    }}

    function getPolicy() public pure returns (string memory) {{
        return "personRole,specialization,designation,fromNetworks,department,experienceYears,certifications,researchFocus,authorizedBy";
    }}

    function evaluate(
        string memory datasetID,
        string[] memory inputs,
        string memory term0,string memory term1,string memory term2,string memory term3,string memory term4,string memory term5,string memory term6,
        bytes memory signature
    ) public {{
        // Verifying the Signature first:
        string memory concatenatedInputs = concatenateInputs(inputs, term0,term1,term2,term3,term4,term5,term6);
        require(
            verify(
                publicKey,
                concatenatedInputs,
        ...
        string[] memory term_values = new string[](7);
        string[] memory term_hashes = new string[](7);
        ...
        (term_hashes[6], term_values[6]) = extractValueAndHash(term6);
    ...
    function concatenateInputs(string[] memory inputs, string memory term0,string memory term1,string memory term2,string memory term3,string memory term4,string memory term5,string memory term6) internal pure returns (string memory) {{
        ...
        for (uint i = 0; i < inputs.length; i++) {{
            ...
            concatenatedInputs = string(abi.encodePacked(concatenatedInputs, ",", term6));
    ...
    function evaluatePolicies(
        ...
        return checkArray(datasetPolicies["personRole"], inputs[0]) && checkArray(datasetPolicies["specialization"], inputs[1]) && checkArray(datasetPolicies["designation"], inputs[2]) && checkArray(datasetPolicies["fromNetworks"], inputs[3]) && checkArray(datasetPolicies["department"], inputs[4]) && stringToUint(datasetPolicies["experienceYears"]) <= stringToUint(inputs[5]) && checkArray(datasetPolicies["certifications"], inputs[6]) && checkArray(datasetPolicies["researchFocus"], inputs[7]) && checkArray(datasetPolicies["authorizedBy"], inputs[8]) && keccak256(abi.encodePacked(datasetPolicies[term_hashes[0]])) == keccak256(abi.encodePacked(term_values[0])) && keccak256(abi.encodePacked(datasetPolicies[term_hashes[1]])) == keccak256(abi.encodePacked(term_values[1])) && keccak256(abi.encodePacked(datasetPolicies[term_hashes[2]])) == keccak256(abi.encodePacked(term_values[2])) && keccak256(abi.encodePacked(datasetPolicies[term_hashes[3]])) == keccak256(abi.encodePacked(term_values[3])) && keccak256(abi.encodePacked(datasetPolicies[term_hashes[4]])) == keccak256(abi.encodePacked(term_values[4])) && keccak256(abi.encodePacked(datasetPolicies[term_hashes[5]])) == keccak256(abi.encodePacked(term_values[5])) && keccak256(abi.encodePacked(datasetPolicies[term_hashes[6]])) == keccak256(abi.encodePacked(term_values[6]));
    }}
    ...
    function generateDecision(
        ...
        if (!permit) {{
            string[] memory failedTerms = new string[](7);
            uint count = 0;
            for (uint i = 0; i < 7; i++) {{
